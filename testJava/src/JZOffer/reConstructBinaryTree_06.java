...
public class reConstructBinaryTree_06 {
    ...
    //由后，中重构树
    public TreeNode reconstruct2(int[] pre,int[] in){
        TreeNode root = reconstruct2(pre,0,pre.length-1,in,0,in.length-1);
        return root;
    }
    //由后中直接输出前
    ...
    private TreeNode reconstruct(int[] pre,int preSart,int preEnd,int[] in,int inSart,int inEnd){
        if(preSart > preEnd || inSart > inEnd){
            return null;
        }
        TreeNode root = new TreeNode(pre[preSart]);
        //寻找在中序遍历中的位置
    ...
    private TreeNode reconstruct2(int[] after,int afterSart,int afterEnd,int[] in,int inSart,int inEnd){
    ...
    public static void main(String[] args){
        ...
        test.getPre(a,b);
//        TreeNode root = test.reconstruct(a,b);
